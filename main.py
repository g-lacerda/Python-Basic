...
while (enc != 2):
    ...
    if escolha == 1:
        ...
        if a == 0:
            ...
            print('_' * 20)
        enc = int(input('Deseja continuar? \n 1. Sim \n 2. Não \n    -->'))
        ...
        print('_' * 20)
        print('Os valores foram atualizados da última vez em: 10/08/2021, as 13:57')
        n = float(input('Quantos reais você tem? R$'))
        print('Com seus R${} (reais), você consegue ter U${:.2f} (dólares)'.format(n, n / 5.20))
        print('Com seus R${} (reais), você consegue ter €{:.2f} (euros)'.format(n, n / 6.10))
        print('Com seus R${} (reais), você consegue ter £{:.2f} (libras)'.format(n, n / 7.20))
        print('Com seus R${} (reais), você consegue ter ¥{:.2f} (ienes)'.format(n, n / 0.047))
        print('_' * 20)
        enc = int(input('Deseja continuar? \n 1. Sim \n 2. Não \n    -->'))
    ...
    if enc == 2:
        ...
        break
    while (enc < 1):
        enc = int(input('Opção inválida. Tente de novo:'))
        while (enc > 2):
            enc = int(input('Opção inválida. Tente de novo:'))
    while (enc > 2):
        enc = int(input('Opção inválida. Tente de novo:'))
